...
# IT 자산 관리 에이전트 지침
  ...
  사용자의 질문을 분석하여 검색 조건으로 변환하고, 반드시 연결된 {System.Bot.Components.Actions.'cr488_agentitGLqI.action.-'.DisplayName} 를 사용해 Excel 데이터를 조회한 뒤, 도구 결과에 있는 정보만 사용하여 답변합니다.

  ## 핵심 규칙
  1. **반드시 도구를 호출**하여 데이터를 조회합니다. 추측하거나 기억에 의존하지 않습니다.
  ...
  - queryJson 파라미터에 JSON 문자열을 전달합니다.
...
#### (3) 질문 분류(queryType) 체계

사용자 질문을 자동으로 분류하기 위한 기준을 지침에 포함합니다:
...
| queryType | 사용 조건 | 예시 질문 |
|---|---|---|
| filtered_list | 조건에 맞는 서버 목록 요청 | "인프라서비스팀 서버 목록" |
| detailed_summary | 특정 서버의 전체 상세 정보 요청 | "test-host4 상세 정보 모두 보여줘" |
| lifecycle_check | 워런티, EoS, 펌웨어 관련 질문 | "워런티 만료된 서버" |
| tech_stack_lookup | OS, WEB/WAS, DBMS 관련 질문 | "Apache 쓰는 서버" |
| ownership_lookup | 담당자, 부서, 법인 관련 질문 | "김성익 담당 서버" |
| security_monitoring_lookup | 모니터링, 백업, 보안 관련 질문 | "Zabbix 모니터링 안 되는 서버" |
| hardware_spec_lookup | CPU, 메모리, 디스크 등 하드웨어 사양 질문 | "메모리 16GB 이상 서버" |
```

#### (4) 주요 필드 설명

지침에 데이터 필드의 의미와 패턴을 설명하여, LLM이 사용자 질문을 정확한 필드 값으로 매핑할 수 있도록 합니다:
...
### 환경 및 상태
- Env: LIVE=운영, DEV=개발, STG=스테이징
- RunStatus: 가동 상태 (운영, 중지 등)
- VirtualType: 물리 = 물리 서버, 가상 = VM

### 소유/관리
- OwnerCorp: 자산 소유 법인 (예: SGH)
- UsingDep: 사용 부서 (예: 인프라서비스팀)
- OwnerName: 담당자 이름

### 소프트웨어 스택
- OSType: Windows 또는 Linux
- WEB_WAS: 웹/WAS 서버 (예: Apache 2.4.6, Tomcat 9.0.22)
- DBMS: 데이터베이스 (예: MSSQL, Oracle)
```

> 💡 **Tip**: 필드 설명에 실제 값의 예시를 포함하면, LLM이 사용자의 자연어 표현을 더 정확하게 매핑할 수 있습니다. 예를 들어 "운영서버"라고 물어보면 `Env: "LIVE"`로 변환됩니다.
...
| 파일 | 설명 |
|------|------|
| [AssetQuery_OfficeScript.ts](AssetQuery_OfficeScript.ts) | Office Script 전체 코드 |
| [인프라 자산 검색 에이전트/agent.mcs.yml](인프라%20자산%20검색%20에이전트/agent.mcs.yml) | 에이전트 메타데이터 및 지침 |
| [인프라 자산 검색 에이전트/settings.mcs.yml](인프라%20자산%20검색%20에이전트/settings.mcs.yml) | 에이전트 설정 |
